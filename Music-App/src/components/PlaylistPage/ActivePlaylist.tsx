...
import { onSetActiveSongs } from "../../redux/slices/musicSlice";


export const ActivePlaylist = ( ) => {
    
  const { activePlaylist, setActiveSong, activeSong, setActiveSongs, setCurrentSongIndex } = useMusic();

  const handleClickSong = ( track: Datum, songIndex: number ) => {
    setActiveSong(track);  
    setActiveSongs(activePlaylist.tracks.data);
        ...
        <div className="overflow-y-auto overflow-x-hidden block max-h-96 music-scroll">
          <table className="w-full  ">
            ...
            <tbody className="divide-y divide-gray-300/30 h-96 overflow-y-auto  ">
              {activePlaylist.tracks?.data.map((track, songIndex ) => (
                <tr
                  key={track.id}
                  onClick={() => handleClickSong(track, songIndex) }
